async function getWeather(id){
    ...
    try{
        ...
        addTextWidget("Загрузка данных...", appContainer);
...
async function fetchWeatherData(coordinates){
        const [weather, time] = await Promise.all([
            fetch(`https://proxy.cors.sh/https://api.open-meteo.com/v1/forecast?latitude=${coordinates.latitude}&longitude=${coordinates.longitude}&current_weather=true&weathercode`)
            .then((res) =>{
                if (res.ok){
                    return res.json()
                }
            ...
            fetch(`https://proxy.cors.sh/https://timeapi.io/api/Time/current/coordinate?latitude=${coordinates.latitude}&longitude=${coordinates.longitude}`)
            .then((res) => {
                if (res.ok){
                    return res.json()
                }
